...
def _primary_monitor():
    """mss uses index 1 for the first physical display."""
    return sct.monitors[1]


def _backing_scale() -> float:
    """
    Physical pixels per PyAutoGUI point (~2 on Retina Mac, 1 on unscaled displays).
    Derived from primary monitor so stream/click coordinates stay aligned.
    """
...
def get_coordinate_system():
    """Logical dimensions for the selected monitor (matches streamed JPEG size)."""
    mon = _selected_monitor()
    scale = _backing_scale()
    lw, lh = monitor_logical_size(mon, scale)
    return {
        "width": lw,
        "height": lh,
...
def _capture_pil_image():
    """Grab selected monitor and resize to PyAutoGUI/logical size."""
    mon = _selected_monitor()
    sct_img = sct.grab(mon)
    img = Image.frombytes("RGB", sct_img.size, sct_img.bgra, "raw", "BGRX")
    scale = _backing_scale()
    lw, lh = monitor_logical_size(mon, scale)
    if (img.width, img.height) != (lw, lh):
...
async def click(req: dict, token: str = Security(api_key_header)):
    ...
    mon = _selected_monitor()
    scale = _backing_scale()
    gx, gy = stream_coords_to_global(int(x), int(y), mon, scale)
    
    event_data = record_event("click", {"x": x, "y": y, "global_x": gx, "global_y": gy})
    try:
        pyautogui.click(gx, gy, duration=0)
    except Exception as exc:
        logger.exception("pyautogui.click failed (macOS: check Accessibility permission)")
        raise HTTPException(
            status_code=500,
            detail=f"Click failed: {exc}. On macOS enable Accessibility for your terminal/Python.",
        ) from exc
    if event_data:
...
async def type_text(req: dict, token: str = Security(api_key_header)):
    ...
    try:
        ...
        logger.exception("pyautogui.write failed")
        raise HTTPException(
            status_code=500,
            detail=f"Type failed: {exc}. On macOS enable Accessibility for your terminal/Python.",
        ) from exc
    if event_data:
        finalize_event(event_data[0], event_data[1])
...
async def press_key(req: dict, token: str = Security(api_key_header)):
    ...
    try:
        ...
        logger.exception("pyautogui.press failed")
        raise HTTPException(
            status_code=500,
            detail=f"Key failed: {exc}. On macOS enable Accessibility for your terminal/Python.",
        ) from exc
    if event_data:
        finalize_event(event_data[0], event_data[1])
...
async def hotkey(req: dict, token: str = Security(api_key_header)):
    ...
    try:
        ...
        logger.exception("pyautogui.hotkey failed")
        raise HTTPException(
            status_code=500,
            detail=f"Hotkey failed: {exc}. On macOS enable Accessibility for your terminal/Python.",
        ) from exc
    if event_data:
        finalize_event(event_data[0], event_data[1])
